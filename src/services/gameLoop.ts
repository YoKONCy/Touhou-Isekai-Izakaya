...
class GameLoopService {
  ...
  async initializeNewGame() {
    ...
    console.log(
      '[游戏循环] 新游戏已初始化。预填充 NPC 数量:',
      Object.keys(gameStore.state.npcs).length
    );
  }
  ...
  async handleUserAction(userContent: string) {
    ...
    try {
      ...
      const retrievedMemories = await memoryService.retrieve(
        currentSaveSlotId,
        finalUserContent,
      ...
      const predictionMatch = finalStory.match(
        /<prediction_trigger>([\s\S]*?)<\/prediction_trigger>/
      );
      if (predictionMatch) {
        try {
          ...
          finalStory = finalStory
            .replace(/<prediction_trigger>[\s\S]*?<\/prediction_trigger>/g, '')
            .trim();
          this.streamedContent.value = finalStory;
        } catch (e) {
      ...
      if (promiseMatch) {
        try {
          ...
          finalStory = finalStory
            .replace(/<promise_trigger>[\s\S]*?<\/promise_trigger>/g, '')
            .trim();
          this.streamedContent.value = finalStory;
        } catch (e) {
      ...
      const managementMatch = finalStory.match(
        /<management_trigger>([\s\S]*?)<\/management_trigger>/
      );
      if (managementMatch) {
        try {
          ...
          finalStory = finalStory
            .replace(/<management_trigger>[\s\S]*?<\/management_trigger>/g, '')
            .trim();
          this.streamedContent.value = finalStory;
        } catch (e) {
      ...
      if (
        ...
        questTriggerData.quests &&
        questTriggerData.quests.length > 0
      ) {
        const gameStore = useGameStore();
        // 将第一个任务设为待处理
      ...
      if (
        questUpdateData &&
        (questUpdateData.update || questUpdateData.status || questUpdateData.log) &&
        questUpdateData.quest_name
      ) {
        const gameStore = useGameStore();
        // 通过名称查找任务，因为 LLM 不知道 ID
        // 我们检查名称匹配并且是 active（进行中）的任务
        const quest = gameStore.state.system.quests?.find(
          (q) => q.name === questUpdateData.quest_name && q.status === 'active'
        );
      ...
      if (
        promiseTriggerData &&
        promiseTriggerData.trigger &&
        promiseTriggerData.promises &&
        promiseTriggerData.promises.length > 0
      ) {
        const gameStore = useGameStore();
      ...
      if (promiseUpdateData && promiseUpdateData.update && promiseUpdateData.content_keyword) {
        ...
        // 模糊匹配内容
        const targetPromise = promises.find(
          (p) => p.status === 'active' && p.content.includes(promiseUpdateData.content_keyword)
        );
      ...
      if (
        predictionTriggerData &&
        predictionTriggerData.trigger &&
        Array.isArray(predictionTriggerData.next_round_characters)
      ) {
  ...
  private async processBackgroundTasks(
    ...
    try {
      ...
      const logicInputSnapshot = JSON.stringify(
        {
          current_state: {
            ...
            scene_npcs: gameStore.state.system.current_scene_npcs.map(
              (id: string) => gameStore.state.npcs[id] || { id, name: id }
            )
          },
      ...
      // LLM #5: 绘图提取（异步）
      // 即发即弃 - 不要为此阻塞后台处理状态
      const currentSceneCharacters = gameStore.state.system.current_scene_npcs.map(
        (id: string) => gameStore.state.npcs[id] || { id, name: id }
      );
  ...
  private initializeCombat(triggerData: any) {
    ...
    const createSpellCard = (name: string): SpellCard => {
      ...
      const preset =
        PRESET_SPELLCARDS[name] || Object.values(PRESET_SPELLCARDS).find((p) => p.name === name);
      if (preset) {
        return {
          name: preset.name!,
      ...
      // 1. 尝试在设定集（静态数据库）中查找
      const cardEntry = charStore.characters.find(
        (c) => c.type === 'spell_card' && (c.name === name || c.name.includes(name))
      );

      if (cardEntry) {
        // 解析设定集数据（数值现在直接存储在数据库中）
        const cost =
          typeof cardEntry.cost === 'number'
            ? cardEntry.cost
            : parseInt(String(cardEntry.cost || '100').replace(/\D/g, '')) || 100;
        const dmg =
          typeof cardEntry.damage === 'number'
            ? cardEntry.damage
            : parseInt(String(cardEntry.damage || '0').replace(/\D/g, '')) || 0;
        ...
        return {
          ...
          hitRate:
            cardEntry.hitRate !== undefined ? cardEntry.hitRate : cardEntry.isUltimate ? 1.0 : 0.1,
    ...
    const findPresetCards = (npcId: string, npcName?: string): SpellCard[] => {
      ...
      if (matches.length === 0 && npcName) {
        ...
        let mappedId =
          CHARACTER_NAME_TO_ID_MAP[npcName] || CHARACTER_NAME_TO_ID_MAP[npcName.trim()];

        // 2.1 模糊搜索（子字符串）- 如果精确匹配失败
        if (!mappedId) {
          const cleanName = npcName.trim();
          // 查找属于名称一部分的任意键，或者名称包含键
          const foundKey = Object.keys(CHARACTER_NAME_TO_ID_MAP).find(
    ...
    if (triggerData.player_buff_name) {
      const buffValue =
        typeof triggerData.player_buff_value === 'number'
          ? triggerData.player_buff_value
          : undefined;
    ...
    const enemies: Combatant[] = (triggerData.enemies || []).map((e: any, index: number) => {
      ...
      if (matchedNPC && matchedNPC.hp) {
        ...
        // 基于战力等级的启发式算法
        if (['EX', 'UX', 'OMEGA', '∞'].includes(powerLevel)) {
          maxHp = 50000;
          hp = 50000;
        } else if (['S', 'S+', 'SS', 'SSS', 'US'].includes(powerLevel)) {
          maxHp = 10000;
          hp = 10000;
        } else if (['A', 'A+', 'B+', 'B'].includes(powerLevel)) {
          maxHp = 5000;
          hp = 5000;
        } else {
          maxHp = 2000;
