...
export const TALENT_MODIFIERS: Record<string, CombatModifier> = {
  ...
  c_preparation: {
    ...
    onCombatStart: (combatant: Combatant, ctx: CombatContext) => {
      ...
      if (ctx.addLog)
        ctx.addLog(`${combatant.name} 触发【快速整备】，回复了 ${healAmount} 点生命值！`);
    }
  },
  c_willpower: {
    id: 'c_willpower',
  ...
  c_shield: {
    ...
    onCombatStart: (combatant: Combatant, ctx: CombatContext) => {
      ...
      if (ctx.addLog)
        ctx.addLog(`${combatant.name} 触发【灵力护盾】，获得 ${shieldAmount} 点护盾！`);
    }
  },
  c_first_aid: {
    id: 'c_first_aid',
    ...
    onCombatWin: (combatant: Combatant, ctx: CombatContext) => {
      ...
      if (ctx.addPopup) ctx.addPopup(combatant, healAmount, 'heal');
      if (ctx.addLog)
        ctx.addLog(`${combatant.name} 触发【战场急救】，战斗胜利后回复 ${healAmount} 点生命值！`);
    }
  },
  c_overload: {
    id: 'c_overload',
  ...
  c_bomb_master: {
    ...
    onCalculateMpCost: (val: number, ctx: CombatContext) => {
      if (ctx.actionType === 'ultimate' || ctx.spellType === 'ultimate')
