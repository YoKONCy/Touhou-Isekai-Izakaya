...
const hasPendingTriggers = computed(() => {
  return (
    !!gameStore.state.system.pending_quest_trigger || !!gameStore.state.system.combat?.isPending
  );
});
  ...
  try {
    ...
    if (dbInfo.type !== 'opfs') {
      ...
      if (!diag.hasGetDirectory) {
        console.warn(
          'OPFS Missing: If on Android, please update "Android System WebView" via Play Store. If on iOS, ensure iOS 15.2+.'
        );
        toastStore.addToast(
          '建议：安卓用户请在应用商店更新 "Android System WebView"，或使用最新版 Chrome/Edge 浏览器。',
          'info',
          15000
        );
        ...
        if (Capacitor.isNativePlatform()) {
          toastStore.addToast(
            '检测到正在使用打包版本：如果多次刷新仍报错，请尝试在手机设置中清除应用缓存并重新打开。',
            'info',
            15000
          );
        } else {
          toastStore.addToast(
            '建议：请检查是否开启了无痕模式，或某些扩展拦截了安全响应头。',
            'info',
            15000
          );
        }
...
async function handleRefresh() {
  ...
  if (textToResend && msgToDeleteId !== undefined) {
    const confirmed = await confirm(
      '确定要重新开始本轮对话吗？这将回滚到此轮开始前的状态并重新生成回复。',
      {
        title: '重新开始对话',
        confirmText: '确定',
        cancelText: '取消',
...
<template>
  ...
  <div
    v-if="isMigrating"
    class="fixed inset-0 z-[100] bg-black/90 flex flex-col items-center justify-center text-white p-8"
  >
    <Loader2 class="w-16 h-16 animate-spin mb-4 text-primary-400" />
    <h2 class="text-2xl font-bold mb-2">正在升级数据库...</h2>
    <p class="text-gray-300 mb-6">
      为了提升性能，我们正在将存档迁移到新数据库。<br />请勿关闭页面，这可能需要几分钟。
    </p>
    ...
    <p class="text-sm text-gray-400">
      {{ migrationMessage }} ({{ Math.floor(migrationProgress) }}%)
    </p>
  </div>
  ...
  <div class="fixed inset-0 flex flex-col overflow-hidden font-sans text-ink bg-izakaya-paper">
    ...
    >
      ...
      <div
        class="font-display font-bold text-xl flex items-center gap-2 md:gap-3 relative z-10 text-izakaya-wood"
      >
        <img
          src="/assets/logo.png"
          alt="Logo"
          class="w-8 h-8 md:w-10 md:h-10 object-contain filter drop-shadow-sm"
        />
      ...
      <div class="hidden md:flex items-center gap-2 relative z-10">
        ...
        >
          <Save
            class="w-5 h-5"
            :class="{ 'text-touhou-red animate-pulse': gameStore.multiplayer.isMultiplayer }"
          />
          <span
            v-if="saveStore.currentSaveId"
            class="text-sm font-display font-medium max-w-[100px] truncate hidden md:inline-block"
          >
        ...
        >
          ...
          <span
            class="absolute -top-1 -right-1 w-2 h-2 bg-touhou-red rounded-full animate-pulse shadow-sm"
          ></span>
        </button>
        <button
          @click="
            ...
            audioManager.playPageFlip();
          "
          class="btn-touhou-ghost"
          title="条目编辑器 (Lorebook)"
        >
          <Book class="w-5 h-5" />
        </button>
        <button
    ...
    <HelpModal
      :is-open="isHelpOpen"
      :initial-section-id="helpInitialSectionId"
    ...
    <SummaryModal
      ...
      :turn-count="summaryTurnCount"
    />
    <MultiplayerHub :is-open="isMultiplayerHubOpen" @close="isMultiplayerHubOpen = false" />
    <DecisionOverlay
      :is-open="isDecisionOverlayOpen"
      @close="isDecisionOverlayOpen = false"
      @submit="handleMultiplayerSubmit"
    />
    ...
    <div
      v-if="mpStatusVisible"
      class="fixed top-24 left-1/2 -translate-x-1/2 z-[70] animate-fade-in-down pointer-events-none"
    >
      <div
        class="bg-izakaya-paper/90 backdrop-blur border border-izakaya-wood/30 px-6 py-3 rounded-full shadow-xl flex items-center gap-3"
      >
    ...
    <div class="flex-1 flex overflow-hidden relative z-10">
      ...
      <div class="flex-1 flex flex-col relative min-w-0 md:hidden">
        ...
        >
          ...
          <div
            ref="chatContainer"
            class="flex-1 overflow-y-auto p-3 space-y-4 scroll-smooth overscroll-contain"
            style="-webkit-overflow-scrolling: touch"
          >
            <!-- New Player Guide -->
            <NewPlayerGuide
            ...
            <div
              ...
              class="text-center text-izakaya-wood/50 mt-20 flex flex-col items-center gap-4"
            >
            ...
            <div
              v-if="gameLoop.isProcessing.value || isGuestProcessing"
              class="flex gap-4 mb-6 px-4 group/message"
            >
              <div
                class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center shadow-md border-2 border-white/50 relative overflow-hidden bg-touhou-red text-white"
              >
              ...
              <div class="relative max-w-[85%] flex flex-col items-start">
                <div
                  class="p-5 rounded-2xl shadow-sm leading-relaxed relative bg-izakaya-paper border border-touhou-red/10 text-izakaya-wood font-serif-display rounded-tl-sm shadow-paper-hover"
                >
                  <!-- Texture -->
                  <div
                    class="absolute inset-0 pointer-events-none opacity-10 bg-texture-rice-paper rounded-2xl"
                  ></div>
                  ...
                  <div class="relative z-10 min-w-[60px]">
                    ...
                    <div
                      v-if="
                        (gameLoop.isProcessing.value &&
                          gameLoop.currentStage.value === 'preparing') ||
                        (isGuestProcessing && guestStage === 'preparing')
                      "
                      class="text-sm text-izakaya-wood/60 animate-pulse flex items-center gap-2"
                    >
                    ...
                    <div
                      v-else-if="
                        (gameLoop.isProcessing.value &&
                          gameLoop.currentStage.value === 'generating_story') ||
                        (isGuestProcessing && guestStage === 'generating_story')
                      "
                    >
                      <div
                        v-if="gameLoop.isProcessing.value && smoothContent"
                      ...
                      ></div>
                      <div
                        v-else-if="isGuestProcessing && guestStreamedContent"
                    ...
                    <div
                      v-else-if="
                        (gameLoop.isProcessing.value &&
                          gameLoop.currentStage.value === 'background_processing') ||
                        (isGuestProcessing && guestStage === 'background_processing')
                      "
                      class="text-sm text-blue-500 flex items-center gap-2"
                    >
            ...
            <div
              v-if="gameLoop.isBackgroundProcessing.value"
              class="flex gap-4 mb-6 px-4 group/message"
            >
              <div
                class="flex-shrink-0 w-10 h-10 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center animate-pulse border-2 border-blue-100 shadow-md"
              >
              ...
              <div class="relative max-w-[85%] flex flex-col items-start">
                <div
                  class="p-5 rounded-2xl shadow-sm leading-relaxed relative bg-white/95 border border-blue-100 text-blue-800 rounded-tl-sm shadow-paper-hover"
                >
                  <div class="text-sm">
                    ...
                    <div class="text-xs text-blue-500 opacity-80">
                      请稍等，处理完成后即可继续对话
                    </div>
                  </div>
                </div>
            ...
            <!-- Error Message -->
            <div
              v-if="gameLoop.error.value"
              class="text-center p-2 text-sm text-red-600 bg-red-50 rounded border border-red-200 mx-4 shadow-sm"
            >
          ...
          <!-- Input Area (Mobile) -->
          <div
            class="p-3 bg-izakaya-paper border-t-2 border-izakaya-wood/20 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] relative z-20 flex-shrink-0"
          >
            <!-- Texture -->
            <div
              ...
              <div class="flex items-end gap-2">
                ...
                <div class="relative flex-1 flex items-end gap-2">
                  <template v-if="gameStore.multiplayer.isMultiplayer">
                    <button
                      @click="
                        isDecisionOverlayOpen = true;
                        audioManager.playPageFlip();
                      "
                      :class="[
                        'w-full py-3 rounded-lg shadow-md transition-all flex items-center justify-center gap-2 font-display',
                        mpAllReady
                          ? 'bg-green-600 text-white'
                          : 'bg-touhou-red text-white hover:bg-touhou-red-dark'
                      ]"
                    >
                      ...
                      <span>跑团决策系统</span>
                      <div
                        v-if="mpTotalPlayers > 0"
                        class="px-2 py-0.5 bg-black/20 rounded-full text-xs font-mono"
                      >
                  ...
                  <template v-else>
                    <div class="relative flex-1">
                      <textarea
                        ...
                        "
                        :placeholder="
                          gameLoop.isBackgroundProcessing.value ? '处理中...' : '输入行动...'
                        "
                        rows="1"
      ...
      <main class="hidden md:flex flex-1 flex-col relative min-w-0">
        ...
        <div ref="chatContainer" class="flex-1 overflow-y-auto p-4 space-y-6 scroll-smooth">
          ...
          <!-- Load Future Button -->
          <div
            v-if="chatStore.hasMoreFuture"
            class="flex flex-col items-center gap-3 py-4 border-t border-izakaya-wood/5 mt-4"
          >
            <button
          ...
          <div v-if="gameLoop.isProcessing.value" class="flex gap-4 mb-6 px-4 group/message">
            ...
            <div class="relative max-w-[85%] flex flex-col items-start">
              ...
              >
                ...
                <div class="relative z-10 min-w-[60px]">
                  <div
                    v-if="gameLoop.currentStage.value === 'preparing'"
                    class="text-sm text-izakaya-wood/60 animate-pulse flex items-center gap-2"
                  >
                    <Loader2 class="w-3 h-3 animate-spin" />
                    <span>正在构建上下文...</span>
                  ...
                  <div
                    v-else-if="gameLoop.currentStage.value === 'background_processing'"
                    class="text-sm text-blue-500 flex items-center gap-2"
                  >
                    <Brain class="w-3 h-3 animate-pulse" />
                    <span>正在处理游戏逻辑...</span>
        ...
        <!-- Desktop Input Area -->
        <div
          class="p-6 bg-izakaya-paper border-t-4 border-izakaya-wood/20 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] relative z-20"
        >
          <!-- Texture -->
          <div
            class="absolute inset-0 pointer-events-none opacity-40 bg-texture-rice-paper mix-blend-multiply"
          ...
          <div class="relative max-w-4xl mx-auto space-y-3">
            ...
            <div v-if="hasPendingTriggers" class="flex flex-wrap gap-3 mb-3 animate-fade-in">
              ...
              >
                ...
                <div class="text-left">
                  <div class="text-xs font-bold opacity-80 uppercase tracking-tighter">
                    发现战斗遭遇
                  </div>
            ...
            >
              ...
              >
                <div
                  class="absolute inset-0 bg-touhou-red/5 group-hover:bg-transparent transition-colors"
                ></div>
                {{ reply }}
              </button>
            </div>
            ...
            <div class="flex items-end gap-3">
              ...
              <div class="relative group h-14 flex-1 flex items-end gap-3">
                ...
                <template v-else>
                  <div class="relative flex-1">
                    <textarea
                      ...
                      :class="[
                        isInputFocused || userInput
                          ? 'h-40 bg-izakaya-paper shadow-[-4px_-4px_15px_rgba(0,0,0,0.1)] rounded-t-lg border-2 border-izakaya-wood/30 z-30'
                          : 'h-14 bg-white/50 border-b-2 border-izakaya-wood/30 z-10'
                      ]"
                    ></textarea>
                  </div>
                </template>
                ...
                >
                  ...
                  >
                    <RefreshCw
                      class="w-5 h-5 group-hover/refresh:rotate-180 transition-transform duration-500"
                    />
                  </button>
                  ...
                  >
                    <Send
                      class="w-5 h-5 group-hover/send:translate-x-0.5 group-hover/send:-translate-y-0.5 transition-transform"
                    />
                  </button>
                  <button
                    v-else-if="gameLoop.isProcessing.value"
              ...
              >
                ...
                <!-- Tooltip / Label -->
                <span
                  class="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap text-xs font-bold px-2 py-1 rounded bg-izakaya-wood text-izakaya-paper opacity-0 group-hover/regen:opacity-100 transition-opacity pointer-events-none shadow-sm"
                >
                ...
                <span
                  v-if="shouldRegenerateMap"
                  class="absolute top-0 right-0 w-2.5 h-2.5 bg-touhou-red rounded-full border border-white"
                ></span>
              </button>
              ...
              >
                ...
                <!-- Tooltip / Label -->
                <span
                  class="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap text-xs font-bold px-2 py-1 rounded bg-izakaya-wood text-izakaya-paper opacity-0 group-hover/mgmt:opacity-100 transition-opacity pointer-events-none shadow-sm"
                >
                  营业系统: 开发中
                </span>
              </button>
      ...
      <aside
        class="w-80 bg-izakaya-paper/60 backdrop-blur-md border-l border-izakaya-wood/10 p-4 overflow-y-auto hidden lg:flex flex-col gap-4 shadow-[-2px_0_10px_rgba(0,0,0,0.02)]"
      >
        <MapPlaceholder
          @click="
            isMapOpen = true;
