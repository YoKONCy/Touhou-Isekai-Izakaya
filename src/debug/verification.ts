...
export function runCombatVerification() {
  console.log(
    '%c--- Running Combat Logic Verification ---',
  ...
  try {
    ...
    console.warn(
      '⚠️ Base Damage Test skipped due to store dependency (likely normal in test env):',
      e
    );
  }
  ...
  if (hpChanged && dotDefender.hp === initialHp - 10) {
    console.log(
      `%c✅ DoT Hook & Sync Logic Verified (HP: ${initialHp} -> ${dotDefender.hp})`,
      'color: green'
    );
  } else {
