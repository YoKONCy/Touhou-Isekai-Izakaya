...
export const useCharacterStore = defineStore('character', () => {
  ...
  async function initializeDefaults() {
    ...
    const spellModules = import.meta.glob('@/assets/lorebook/spellcards/**/*.json', { eager: true });
    ...
    for (const path in allModules) {
      ...
      if (existing && existing.id) {
        ...
        const fieldsToSync = ['gender', 'initialPower', 'initialMaxHp', 'initialResidence', 'cost', 'damage', 'damageType'];
        
        for (const field of fieldsToSync) {
            if ((existing[field as keyof CharacterCard] === undefined || existing[field as keyof CharacterCard] === '' || existing[field as keyof CharacterCard] === 0) && charData[field] !== undefined) {
                missingFields[field] = charData[field];
            }
        }

        const needsUpdate = existing.type !== inferredType || 
                          (existing.category !== inferredCategory && inferredCategory !== '未分类' && inferredCategory !== '地点') ||
