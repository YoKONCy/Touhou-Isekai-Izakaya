...
const canProceed = computed(() => {
  ...
  if (currentStep.value.id === 'store_staff') {
    ...
    if (
      formData.store.staffBackstoryType === 'custom' &&
      formData.store.staffBackstory.trim().length === 0
    )
      return false;
    return true;
  }
...
<template>
  <div
    class="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/80 backdrop-blur-md p-4 animate-in fade-in duration-300"
  >
    <!-- Main Wizard Card -->
    <div
    ...
    >
      ...
      >
        <div>
          <h2 class="text-2xl font-bold text-izakaya-wood dark:text-stone-200 font-serif">
            幻想乡入境登记
          </h2>
          <p class="text-sm text-izakaya-wood/70 dark:text-stone-400 mt-1">
            请填写您的个人档案以生成签证。
          </p>
        </div>
        <button
      ...
      <div class="relative z-10 flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar">
        ...
        <div
          v-if="currentStep.id === 'difficulty_select'"
          class="space-y-6 animate-in slide-in-from-right-4 duration-300"
        >
          <h3
            class="text-lg font-medium text-izakaya-wood dark:text-stone-200 mb-4 flex items-center gap-2"
          >
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div
              ...
              :class="
                formData.difficulty === 'gentle'
                  ? 'border-green-500 bg-green-500/5'
                  : 'border-izakaya-wood/20 bg-white/40 dark:bg-stone-800/40 hover:border-green-500/50'
              "
            >
              <div
                class="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 bg-green-500/5 transition-opacity duration-300"
              ></div>
              <div
                class="p-4 bg-green-100 dark:bg-green-900/30 rounded-full text-green-600 dark:text-green-400 relative z-10 transition-transform group-hover:scale-110 duration-300"
              >
                <BookUser class="w-8 h-8" />
              </div>
              <div class="relative z-10">
                <h4 class="font-bold text-lg mb-2 text-izakaya-wood dark:text-stone-200">
                  温柔的世界
                </h4>
                <p class="text-sm text-izakaya-wood/70 dark:text-stone-400">
                  世界对你充满善意。NPC更加友好，战斗更加轻松。
                </p>
              </div>
            </div>
            ...
            <div
              ...
              :class="
                formData.difficulty === 'normal'
                  ...
                  : 'border-izakaya-wood/20 bg-white/40 dark:bg-stone-800/40 hover:border-blue-500/50'
              "
            >
              <div
                class="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 bg-blue-500/5 transition-opacity duration-300"
              ></div>
              <div
                class="p-4 bg-blue-100 dark:bg-blue-900/30 rounded-full text-blue-600 dark:text-blue-400 relative z-10 transition-transform group-hover:scale-110 duration-300"
              >
                <User class="w-8 h-8" />
              </div>
              <div class="relative z-10">
            ...
            <div
              ...
              class="group relative p-6 rounded-xl border-2 cursor-pointer transition-all hover:shadow-lg flex flex-col gap-4 text-center items-center overflow-hidden"
              :class="
                formData.difficulty === 'cruel'
                  ? 'border-red-600 bg-red-600/5'
                  : 'border-izakaya-wood/20 bg-white/40 dark:bg-stone-800/40 hover:border-red-600/50'
              "
            >
              <div
                class="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 bg-red-600/5 transition-opacity duration-300"
              ></div>
              <div
                class="p-4 bg-red-100 dark:bg-red-900/30 rounded-full text-red-600 dark:text-red-400 relative z-10 transition-transform group-hover:scale-110 duration-300"
              >
              ...
              <div class="relative z-10">
                ...
                <p class="text-sm text-izakaya-wood/70 dark:text-stone-400">
                  在这个充满恶意的世界里，生存本身就是一种挑战。NPC更加冷漠，战斗异常艰难。
                </p>
              </div>
            </div>
          </div>
        ...
        <div
          v-if="currentStep.id === 'mode_select'"
          class="space-y-6 animate-in slide-in-from-right-4 duration-300"
        >
          <h3
            class="text-lg font-medium text-izakaya-wood dark:text-stone-200 mb-4 flex items-center gap-2"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              <div
                class="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 bg-touhou-red/5 transition-opacity duration-300"
              ></div>
              <div
                class="p-4 bg-red-100 dark:bg-red-900/30 rounded-full text-touhou-red relative z-10 transition-transform group-hover:scale-110 duration-300"
              >
                <BookUser class="w-8 h-8" />
              </div>
              <div class="relative z-10">
          ...
          <!-- Store Option Toggle -->
          <div
            class="pt-4 border-t border-izakaya-wood/10 dark:border-stone-700 animate-in fade-in slide-in-from-bottom-2 duration-500"
          >
            <div
              ...
              :class="
                isStoreStart
                  ...
                  : 'border-izakaya-wood/20 bg-white/40 dark:bg-stone-800/40 hover:border-amber-500/50'
              "
            >
              <div class="flex items-center gap-4">
                <div
                  class="p-3 bg-amber-100 dark:bg-amber-900/30 rounded-lg text-amber-600 dark:text-amber-400 group-hover:scale-110 transition-transform"
                >
                  <Store class="w-6 h-6" />
                </div>
                <div>
                  <h4 class="font-bold text-base text-izakaya-wood dark:text-stone-200">
                    店铺开局 (经营模式)
                  </h4>
                  <p class="text-sm text-izakaya-wood/70 dark:text-stone-400">
                    选择此项将在开局拥有一家店铺，并自动生成相关的经营剧情。
                  </p>
                </div>
              </div>
              <div
                class="relative w-12 h-6 bg-gray-200 dark:bg-stone-700 rounded-full transition-colors duration-300"
                :class="{ 'bg-amber-500': isStoreStart }"
              >
                <div
                  class="absolute top-1 left-1 w-4 h-4 bg-white rounded-full shadow transition-transform duration-300"
                  :class="{ 'translate-x-6': isStoreStart }"
                ></div>
              </div>
            </div>
          </div>
        ...
        <!-- Step: Store Config -->
        <div
          v-if="currentStep.id === 'store_config'"
          class="space-y-6 animate-in slide-in-from-right-4 duration-300"
        >
          <h3
            class="text-lg font-medium text-izakaya-wood dark:text-stone-200 mb-4 flex items-center gap-2"
          ...
          <div class="space-y-4">
            <div class="space-y-2">
              <label class="block text-sm font-medium text-izakaya-wood dark:text-stone-300"
                >店铺名称</label
              >
              <input
            ...
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-2">
                <div
                  ...
                  :class="
                    formData.store.location === loc
                      ? 'bg-amber-500/10 border-amber-500 text-amber-600'
                      : 'border-izakaya-wood/20 hover:bg-izakaya-wood/5 dark:border-stone-600 dark:text-stone-300'
                  "
                >
                  {{ loc }}
                </div>
            ...
            <div class="space-y-2">
              <label class="block text-sm font-medium text-izakaya-wood dark:text-stone-300"
                >店铺介绍</label
              >
              <p class="text-xs text-izakaya-wood/60 dark:text-stone-500 mb-2">
                请描述店铺的经营内容、内部构造、已有设施等。
              </p>
              <textarea
        ...
        <!-- Step: Store Staff -->
        <div
          v-if="currentStep.id === 'store_staff'"
          class="space-y-6 animate-in slide-in-from-right-4 duration-300"
        >
          <h3
            class="text-lg font-medium text-izakaya-wood dark:text-stone-200 mb-4 flex items-center gap-2"
          ...
          >
            ...
            <div
              ...
              :class="{ 'bg-amber-500': formData.store.hasStaff }"
            >
              <div
                class="absolute top-1 left-1 w-3 h-3 bg-white rounded-full shadow transition-transform duration-300"
                :class="{ 'translate-x-5': formData.store.hasStaff }"
              ></div>
            </div>
          </div>

          <div
            v-if="formData.store.hasStaff"
            ...
            <div class="space-y-2">
              ...
              <div class="flex gap-4 mb-2">
                <label class="flex items-center gap-2 text-sm cursor-pointer">
                  <input
                    type="radio"
                    v-model="formData.store.staffBackstoryType"
                    value="custom"
              ...
              <div
                v-else
                class="p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 rounded-lg text-sm text-amber-800 dark:text-amber-200 flex items-center gap-2"
              >
                <Brain class="w-4 h-4" />
        ...
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div
              ...
              @click="
                () => {
                  formData.originId = origin.id;
                  formData.presetLocation = origin.stats.location;
                }
              "
            ...
            >
              <div class="flex items-center gap-3 relative z-10">
                <div
                  class="p-2 rounded-lg bg-izakaya-wood/5 dark:bg-stone-700 text-izakaya-wood dark:text-stone-300 group-hover:text-touhou-red transition-colors"
                >
              ...
              <p class="text-sm text-izakaya-wood/70 dark:text-stone-400 mt-2 relative z-10">
                {{ origin.desc }}
              </p>
              <div
                class="mt-auto pt-4 flex gap-2 text-xs font-mono text-izakaya-wood/60 flex-wrap relative z-10"
              >
                <span
                  class="bg-izakaya-wood/10 dark:bg-stone-700 px-2 py-1 rounded border border-izakaya-wood/10"
                  >💰 {{ origin.stats.money }}</span
                >
                <span
                  class="bg-izakaya-wood/10 dark:bg-stone-700 px-2 py-1 rounded border border-izakaya-wood/10"
                  >⚔️ {{ origin.stats.power }}</span
                >
                <span
                  class="bg-izakaya-wood/10 dark:bg-stone-700 px-2 py-1 rounded border border-izakaya-wood/10"
                  >❤️ {{ origin.stats.hp }}</span
                >
              </div>
              <div
                v-if="formData.originId === origin.id"
                class="absolute top-4 right-4 text-touhou-red animate-in zoom-in duration-300"
              >
                <Check class="w-5 h-5" />
              </div>
            </div>
        ...
        >
          ...
          <p class="text-sm text-izakaya-wood/70 dark:text-stone-400 mb-4">
            推荐：博丽神社、人间之里
          </p>
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div
              ...
              :class="
                formData.presetLocation === loc
                  ? 'border-touhou-red bg-touhou-red/5 text-touhou-red font-bold'
                  : 'border-izakaya-wood/20 bg-white/40 dark:bg-stone-800/40 text-izakaya-wood/80 dark:text-stone-300 hover:border-touhou-red/50'
              "
            >
        ...
        >
          ...
          <div class="space-y-2">
            ...
            >
            <p class="text-xs text-izakaya-wood/60 dark:text-stone-500 mb-2">
              已选择“{{ selectedOrigin.name }}”模板。你可以在此补充一些性格细节。
            </p>
            <textarea
        ...
        >
          ...
          <div class="space-y-2">
            ...
            >
            <p class="text-xs text-izakaya-wood/60 dark:text-stone-500 mb-2">
              请使用 JSON 格式编写您的详细设定。这将作为本存档世界观的一部分。
            </p>
            <div class="relative">
              <textarea
...
...
              ...
              <div class="absolute top-2 right-2">
                ...
                <button
                  ...
                  "
                  class="text-xs bg-izakaya-wood/10 hover:bg-izakaya-wood/20 dark:bg-stone-600 px-2 py-1 rounded transition-colors text-izakaya-wood dark:text-stone-300"
                >
                  插入模板
                </button>
              </div>
        ...
        >
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1">
              ...
              <input
                v-model.number="formData.customStats.money"
                type="number"
                class="w-full p-2 rounded border border-izakaya-wood/30 dark:border-stone-600 bg-white/50 dark:bg-stone-700 dark:text-stone-100 text-stone-900 focus:border-touhou-red focus:outline-none"
              />
            </div>
            <div class="space-y-1">
              <label class="text-xs text-izakaya-wood/70 dark:text-stone-400">战斗力 (Power)</label>
              <input
                v-model="formData.customStats.power"
                type="text"
                class="w-full p-2 rounded border border-izakaya-wood/30 dark:border-stone-600 bg-white/50 dark:bg-stone-700 dark:text-stone-100 text-stone-900 focus:border-touhou-red focus:outline-none"
              />
            </div>
            <div class="space-y-1">
              ...
              <input
                v-model.number="formData.customStats.hp"
                type="number"
                class="w-full p-2 rounded border border-izakaya-wood/30 dark:border-stone-600 bg-white/50 dark:bg-stone-700 dark:text-stone-100 text-stone-900 focus:border-touhou-red focus:outline-none"
              />
            </div>
            <div class="space-y-1">
              ...
              <input
                v-model.number="formData.customStats.mp"
                type="number"
                class="w-full p-2 rounded border border-izakaya-wood/30 dark:border-stone-600 bg-white/50 dark:bg-stone-700 dark:text-stone-100 text-stone-900 focus:border-touhou-red focus:outline-none"
              />
            </div>
            <div class="space-y-1">
              <label class="text-xs text-izakaya-wood/70 dark:text-stone-400"
                >身份 (Identity)</label
              >
              <input
                v-model="formData.customStats.identity"
                type="text"
                class="w-full p-2 rounded border border-izakaya-wood/30 dark:border-stone-600 bg-white/50 dark:bg-stone-700 dark:text-stone-100 text-stone-900 focus:border-touhou-red focus:outline-none"
              />
            </div>
            <div class="space-y-1">
              ...
              >
              <input
                v-model="formData.customStats.clothing"
                type="text"
                class="w-full p-2 rounded border border-izakaya-wood/30 dark:border-stone-600 bg-white/50 dark:bg-stone-700 dark:text-stone-100 text-stone-900 focus:border-touhou-red focus:outline-none"
              />
            </div>
            <div class="space-y-1" v-if="!isStoreStart">
              ...
              <input
                v-model="formData.customStats.location"
                type="text"
                class="w-full p-2 rounded border border-izakaya-wood/30 dark:border-stone-600 bg-white/50 dark:bg-stone-700 dark:text-stone-100 text-stone-900 focus:border-touhou-red focus:outline-none"
              />
            </div>
            <div class="space-y-1">
              ...
              <input
                v-model="formData.customStats.time"
                type="text"
                class="w-full p-2 rounded border border-izakaya-wood/30 dark:border-stone-600 bg-white/50 dark:bg-stone-700 dark:text-stone-100 text-stone-900 focus:border-touhou-red focus:outline-none"
              />
            </div>
            <div class="col-span-2 space-y-1">
              ...
              <input
                v-model="formData.customStats.date"
                type="text"
                class="w-full p-2 rounded border border-izakaya-wood/30 dark:border-stone-600 bg-white/50 dark:bg-stone-700 dark:text-stone-100 text-stone-900 focus:border-touhou-red focus:outline-none"
              />
            </div>
        ...
        >
          <div
            class="bg-white/40 dark:bg-stone-800/40 rounded-xl p-6 border border-izakaya-wood/20 dark:border-stone-700 shadow-sm relative overflow-hidden"
          >
            <div
              class="absolute inset-0 pointer-events-none opacity-30 bg-texture-rice-paper"
            ></div>
            <h3
              class="text-lg font-bold text-izakaya-wood dark:text-stone-200 mb-4 flex items-center gap-2 relative z-10"
            >
            ...
            <div class="space-y-4 relative z-10">
              <div
                class="grid grid-cols-3 gap-4 border-b border-izakaya-wood/10 dark:border-stone-700 pb-4"
              >
                ...
                <template v-if="mode === 'preset'">
                  ...
                  <div class="col-span-2 text-sm text-izakaya-wood dark:text-stone-300">
                    {{ formData.presetLocation }}
                  </div>
                </template>
                <template v-else>
                  <div class="text-sm text-izakaya-wood/60 dark:text-stone-500">初始地点</div>
                  <div class="col-span-2 text-sm text-izakaya-wood dark:text-stone-300">
                    {{ formData.customStats.location }}
                  </div>
                </template>
              </div>
              ...
              <div>
                ...
                <div
                  class="p-3 bg-white/60 dark:bg-stone-800/60 rounded-lg text-sm leading-relaxed border border-izakaya-wood/20 dark:border-stone-600 whitespace-pre-wrap font-mono text-xs text-izakaya-wood/80 dark:text-stone-300 custom-scrollbar max-h-40 overflow-y-auto"
                >
                  {{
                    mode === 'preset'
                      ? formData.persona || '(使用默认预设描述)'
                      : formData.detailedSetting || '(无详细设定)'
                  }}
                </div>
              </div>
              ...
              <div
                v-if="isStoreStart"
                class="mt-4 border-t border-izakaya-wood/10 dark:border-stone-700 pt-4"
              >
                <div
                  class="text-sm font-bold text-amber-600 dark:text-amber-400 mb-2 flex items-center gap-2"
                >
