  ...
  >
    ...
    <div class="border-b border-izakaya-wood/20 bg-izakaya-wood/5 relative z-10">
      ...
      >
        ...
        <span
          ...
          :class="
            viewMode === 'quests' ? 'opacity-100 scale-105 text-touhou-red' : 'opacity-40 scale-95'
          "
          >任务栏</span
        >
        ...
        <span
          class="transition-all duration-300 relative z-10"
          :class="
            viewMode === 'promises' ? 'opacity-100 scale-105 text-blue-600' : 'opacity-40 scale-95'
          "
          >约定栏</span
        >
        ...
        <div
          ...
          :class="
            viewMode === 'quests' ? 'left-0 w-1/2 bg-touhou-red' : 'left-1/2 w-1/2 bg-blue-500'
          "
        ></div>
      </button>
    </div>
    ...
    <div class="flex border-b border-izakaya-wood/10 bg-white/20 relative z-10">
      <button
        ...
        :class="
          currentTab === 'active'
            ? 'text-touhou-red'
            : 'text-izakaya-wood/50 hover:text-izakaya-wood hover:bg-white/40'
        "
      >
        <span class="relative z-10">进行中</span>
        <div
          v-if="currentTab === 'active'"
          class="absolute bottom-0 left-0 right-0 h-0.5 bg-touhou-red shadow-[0_0_8px_rgba(211,47,47,0.4)]"
        ></div>
      </button>
      <button
      ...
      >
        ...
        <div
          ...
          class="absolute bottom-0 left-0 right-0 h-0.5 bg-izakaya-wood/50"
        ></div>
      </button>
    </div>
      ...
      <template v-if="viewMode === 'quests'">
        ...
        <template v-if="currentTab === 'active'">
          <div
            ...
            class="text-xs text-izakaya-wood/40 py-8 text-center font-serif"
          >
          ...
          >
            <div class="flex justify-between items-start gap-2">
              <div
                class="text-sm font-bold font-display text-izakaya-wood group-hover:text-touhou-red transition-colors"
              >
                {{ quest.name }}
              </div>
              <span
                class="text-[10px] bg-touhou-red/10 text-touhou-red px-1.5 py-0.5 rounded border border-touhou-red/20 whitespace-nowrap"
                >进行中</span
              >
            </div>
        ...
        <template v-else>
          ...
          >
            <div class="flex justify-between items-start">
              <div
                class="text-sm font-medium font-display text-izakaya-wood/60 line-through decoration-izakaya-wood/30"
              >
                {{ quest.name }}
              </div>
              <span
            ...
            <div class="text-xs text-izakaya-wood/40 mt-1 font-serif">
              发布人: {{ quest.giver }}
            </div>
          </div>
        </template>
      ...
      <template v-else>
        ...
        <template v-if="currentTab === 'active'">
          ...
          >
            <div class="flex justify-between items-start gap-2">
              <div
                class="text-sm font-bold font-display text-izakaya-wood group-hover:text-blue-600 transition-colors line-clamp-2"
              >
                {{ promise.content }}
              </div>
              <span
                class="text-[10px] bg-blue-500/10 text-blue-600 px-1.5 py-0.5 rounded border border-blue-500/20 whitespace-nowrap"
                >约定</span
              >
            </div>
            <div
              class="text-xs text-izakaya-wood/60 mt-2 flex items-center gap-1 font-serif justify-between"
            >
        ...
        <template v-else>
          ...
          >
            <div class="flex justify-between items-start">
              <div
                class="text-sm font-medium font-display text-izakaya-wood/60 line-through decoration-izakaya-wood/30 line-clamp-2"
              >
                {{ promise.content }}
              </div>
              <span
    ...
    >
      ...
      >
        ...
        <div
          class="p-5 border-b border-izakaya-wood/10 relative z-10 bg-white/40 flex justify-between items-start"
        >
          <div>
            ...
            <div class="flex items-center gap-2 mt-1">
              <span class="text-xs font-serif text-izakaya-wood/60"
                >发布人: {{ selectedQuest.giver }}</span
              >
              <span
          ...
          <button
            @click="handleCloseQuest"
            class="text-izakaya-wood/40 hover:text-touhou-red transition-colors"
          >
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-5 text-sm relative z-10 custom-scrollbar">
          <div
            class="bg-white/50 p-4 rounded-lg border border-izakaya-wood/5 text-izakaya-wood/80 leading-relaxed font-serif shadow-sm"
          >
            {{ selectedQuest.description }}
          </div>

          <div v-if="selectedQuest.requirements && selectedQuest.requirements.length > 0">
            <div class="font-bold font-display text-izakaya-wood mb-2 text-xs opacity-70">
              具体要求
            </div>
            <ul class="space-y-1">
              <li
                v-for="(req, idx) in selectedQuest.requirements"
                :key="idx"
                class="flex items-start gap-2 text-izakaya-wood/80 text-sm font-serif"
              >
          ...
          <div
            v-if="selectedQuest.completionSummary"
            class="bg-marisa-gold/5 p-4 rounded-lg border border-marisa-gold/20"
          >
            <div class="font-bold font-display text-marisa-gold-dim mb-1 flex items-center gap-2">
              <span class="text-lg">📜</span> 结案
            </div>
            <div class="italic text-izakaya-wood/80 font-serif">
              {{ selectedQuest.completionSummary }}
            </div>
          </div>
          ...
          <div v-if="selectedQuest.logs && selectedQuest.logs.length > 0">
            ...
            <div class="relative pl-3 border-l border-izakaya-wood/10 space-y-4">
              <div v-for="(log, idx) in selectedQuest.logs" :key="idx" class="relative">
                <!-- Dot -->
                <div
                  class="absolute -left-[17px] top-1.5 w-2 h-2 rounded-full bg-izakaya-wood/20 border border-izakaya-paper"
                ></div>

                <div class="text-izakaya-wood/90 text-sm font-serif leading-relaxed">
                  {{ log.content }}
                </div>
          ...
          <div v-if="selectedQuest.rewards && selectedQuest.rewards.length > 0">
            <div
              class="font-bold font-display text-izakaya-wood mb-2 border-b border-izakaya-wood/10 pb-1"
            >
              奖励
            </div>
            <ul class="space-y-2">
              <li
                v-for="(reward, idx) in selectedQuest.rewards"
                :key="idx"
                class="flex items-start gap-2 text-izakaya-wood/80"
              >
                <span class="text-touhou-red mt-0.5">◆</span>
                <span>
                  <span
                    class="text-marisa-gold-dim font-bold text-xs bg-marisa-gold/10 px-1 rounded mr-1"
                    >[{{ getRewardTypeName(reward.type) }}]</span
                  >
                  {{ reward.description }}
                  <span v-if="reward.value" class="text-izakaya-wood/50 text-xs ml-1 font-mono"
                    >({{ formatRewardValue(reward) }})</span
                  >
                </span>
              </li>
            </ul>
          ...
          <div
            class="text-xs text-izakaya-wood/40 pt-4 border-t border-izakaya-wood/10 space-y-1 font-mono"
          >
            <div>接受于第 {{ selectedQuest.acceptedTurn }} 轮</div>
            <div v-if="selectedQuest.completedTurn">
              结束于第 {{ selectedQuest.completedTurn }} 轮
              <span v-if="selectedQuest.completedDate"
                >({{ selectedQuest.completedDate }} {{ selectedQuest.completedTime }})</span
              >
            </div>
          </div>
        </div>
      ...
      >
        ...
        >
          <div>
            ...
            <div class="flex items-center gap-2 mt-1">
              <span class="text-xs font-serif text-izakaya-wood/60"
                >相关方: {{ selectedPromise.giver }}</span
              >
              <span
              ...
              >
                {{
                  selectedPromise.status === 'completed'
                    ? '已达成'
                    : selectedPromise.status === 'expired'
                      ? '已失效'
                      : '已失败'
                }}
              </span>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-5 text-sm relative z-10 custom-scrollbar">
          ...
          >
            ...
            <div
              v-if="selectedPromise.completedTurn"
              class="mt-2 pt-2 border-t border-dashed border-izakaya-wood/10"
            >
