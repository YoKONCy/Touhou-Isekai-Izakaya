...
import { Folder, FileText, Plus, Trash2, Save, X, Edit2, Lock, ArrowLeft, RotateCcw, ShieldAlert, Library, ChevronRight } from 'lucide-vue-next';
...
async function handleFactoryReset() {
  if (await confirm('确定要恢复出厂设置吗？\n这将删除所有手动修改和新建的条目，并重新加载系统默认设置。此操作不可撤销！', { destructive: true })) {
        ...
        >
          ...
          <div class="flex-[0.45] min-h-[180px] flex flex-col overflow-hidden border-b border-izakaya-wood/10">
            ...
            <div class="flex-1 overflow-y-auto px-3 pb-3 space-y-1 custom-scrollbar">
              <div 
                ...
                :class="selectedCategory === cat ? 'bg-touhou-red text-white shadow-md' : 'hover:bg-stone-200 dark:hover:bg-stone-700 text-izakaya-wood dark:text-stone-300'"
                @click="selectedCategory = cat"
              >
                <div class="flex items-center gap-2.5 min-w-0">
                  <Library v-if="cat === '全部'" class="w-4 h-4 flex-shrink-0" />
                  <Folder v-else class="w-4 h-4 flex-shrink-0" :class="selectedCategory === cat ? 'text-white' : 'text-izakaya-wood/40 dark:text-stone-500'" />
                  <span class="text-sm font-bold truncate">{{ cat }}</span>
                </div>
                <div class="flex items-center gap-2">
                  <span 
                    class="text-[10px] font-mono px-1.5 py-0.5 rounded-full"
                    :class="selectedCategory === cat ? 'bg-white/20 text-white' : 'bg-stone-300/50 dark:bg-stone-600/50 text-izakaya-wood/60 dark:text-stone-400'"
                  >
          ...
          <div class="flex-[0.55] flex flex-col overflow-hidden bg-stone-50/30 dark:bg-stone-900/10">
            ...
            <div class="flex-1 overflow-y-auto p-3 space-y-1.5 custom-scrollbar">
              ...
              >
                <div class="w-1.5 h-1.5 rounded-full transition-colors flex-shrink-0" :class="selectedCharId === char.uuid ? 'bg-touhou-red shadow-[0_0_8px_rgba(239,68,68,0.5)]' : 'bg-stone-300 dark:bg-stone-600 group-hover:bg-touhou-red/30'"></div>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between">
                    <span class="font-bold truncate" :class="selectedCharId === char.uuid ? 'text-touhou-red' : 'text-izakaya-wood dark:text-stone-200'">{{ char.name }}</span>
                    <ChevronRight v-if="selectedCharId === char.uuid" class="w-3.5 h-3.5 opacity-50" />
                  </div>
            ...
            <div class="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 md:space-y-8 custom-scrollbar relative" style="-webkit-overflow-scrolling: touch;">
              ...
              <div>
                 <div class="flex items-center justify-between mb-2">
                    ...
                    <span v-if="isTypeDisabled" class="text-[10px] text-touhou-red font-bold flex items-center gap-1 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded">
                       <Lock class="w-3 h-3" /> {{ selectedCharId ? '已锁定 (现有条目不可更改类型)' : '暂时仅支持新增角色类型' }}
                    </span>
                 </div>
                 <div class="flex rounded-lg shadow-sm overflow-hidden border border-izakaya-wood/20 flex-wrap" :class="{'opacity-70 cursor-not-allowed': isTypeDisabled}">
                    <button 
                ...
                <div>
                  ...
                  <div class="relative">
                    ...
                    <datalist id="category-options">
                      <option v-for="cat in charStore.getCategories()" :key="cat" :value="cat">{{ cat }}</option>
                    </datalist>
                  </div>
                </div>
                    ...
                    <div>
                       ...
                       <div class="flex gap-2">
                          <input 
                            ...
                            placeholder="输入唯一标识符..."
                          >
                          <button class="text-xs text-blue-500 hover:text-blue-600 font-bold px-2" @click="copyToClipboard(formData.uuid || '')">复制</button>
                       </div>
                    </div>
                    <div>
              ...
              <div v-if="formData.type === 'spell_card'" class="p-5 bg-blue-50/80 dark:bg-blue-900/10 rounded-xl border border-blue-200 dark:border-blue-800 space-y-4">
                 ...
                 <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                    <div>
                       <label class="block text-xs font-bold text-blue-900/60 dark:text-blue-300/60 mb-1.5">灵力消耗 (Cost)</label>
                       <input v-model.number="formData.cost" type="number" class="w-full text-sm border-blue-200 rounded shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-stone-900 dark:border-blue-800 dark:text-stone-100 text-stone-900 bg-white" placeholder="如: 50">
                    </div>
                    <div>
                       <label class="block text-xs font-bold text-blue-900/60 dark:text-blue-300/60 mb-1.5">伤害数值 (Damage)</label>
                       <input v-model.number="formData.damage" type="number" class="w-full text-sm border-blue-200 rounded shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-stone-900 dark:border-blue-800 dark:text-stone-100 text-stone-900 bg-white" placeholder="如: 500">
                    </div>
                    <div>
                       <label class="block text-xs font-bold text-blue-900/60 dark:text-blue-300/60 mb-1.5">符卡类型</label>
                       <select v-model="formData.damageType" class="w-full text-sm border-blue-200 rounded shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-stone-900 dark:border-blue-800 dark:text-stone-100 text-stone-900 bg-white">
                    ...
                    <div>
                       <label class="block text-xs font-bold text-blue-900/60 dark:text-blue-300/60 mb-1.5">作用范围 (Scope)</label>
                       <select v-model="formData.scope" class="w-full text-sm border-blue-200 rounded shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-stone-900 dark:border-blue-800 dark:text-stone-100 text-stone-900 bg-white">
                 ...
                 <div class="flex items-center gap-6 pt-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                       <input type="checkbox" v-model="formData.isUltimate" class="rounded border-blue-300 text-blue-600 focus:ring-blue-500">
                       <span class="text-xs font-bold text-blue-900/60 dark:text-blue-300/60">终极符卡 (Ultimate)</span>
                    </label>
                    <div class="flex-1 flex items-center gap-2">
                       <label class="text-xs font-bold text-blue-900/60 dark:text-blue-300/60 whitespace-nowrap">命中率 (Hit Rate):</label>
                       <input v-model.number="formData.hitRate" type="range" min="0" max="1" step="0.1" class="flex-1 h-1.5 bg-blue-200 rounded-lg appearance-none cursor-pointer accent-blue-600">
                       <span class="text-xs font-mono text-blue-600 w-8">{{ formData.hitRate || 0 }}</span>
                    </div>
                 </div>
              </div>
