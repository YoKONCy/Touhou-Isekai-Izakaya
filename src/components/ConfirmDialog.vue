...
<template>
  <Teleport to="body">
    <div
      ...
      class="fixed inset-0 z-[200] flex items-center justify-center bg-stone-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200"
    >
      <div
      ...
      >
        ...
        <div class="relative z-10 p-6 text-center">
          ...
          >
            <AlertTriangle
              v-if="confirmState.options.destructive"
              class="w-7 h-7 text-touhou-red"
            />
            <HelpCircle v-else class="w-7 h-7 text-marisa-gold" />
          </div>

          <h3 class="text-xl font-bold text-izakaya-wood dark:text-stone-200 mb-3 font-serif">
            {{ confirmState.options.title }}
          </h3>
          <p
            class="text-sm text-izakaya-wood/80 dark:text-stone-400 whitespace-pre-wrap leading-relaxed font-sans"
          >
        ...
        <div
          class="relative z-10 flex border-t border-izakaya-wood/10 dark:border-stone-700 bg-izakaya-wood/5 dark:bg-stone-800/50"
        >
          <button
