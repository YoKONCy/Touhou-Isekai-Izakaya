...
const RANKS = ['∞', 'OMEGA', 'UX', 'EX', 'US', 'SSS', 'SS', 'S+', 'S', 'A+', 'A', 'B+', 'B', 'C+', 'C', 'D+', 'D', 'E+', 'E', 'F+', 'F', 'F-'];
...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
    <div class="bg-izakaya-paper w-full max-w-2xl rounded-xl shadow-2xl flex flex-col max-h-[90vh] border border-izakaya-wood/20 relative overflow-hidden">
        <!-- Texture -->
        ...
        <div class="flex border-b border-izakaya-wood/10 bg-white/20 relative z-10">
            <button 
                @click="activeTab = 'custom'; audioManager.playSoftClick()"
                class="flex-1 py-3 text-sm font-bold transition-colors flex items-center justify-center gap-2 relative"
                :class="activeTab === 'custom' ? 'text-touhou-red bg-white/60' : 'text-izakaya-wood/60 hover:bg-white/40'"
            >
                <Edit3 class="w-4 h-4" />
                自定义详情
                <div v-if="activeTab === 'custom'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-touhou-red"></div>
            </button>
            <button 
                @click="activeTab = 'preset'; audioManager.playSoftClick()"
                class="flex-1 py-3 text-sm font-bold transition-colors flex items-center justify-center gap-2 relative"
                :class="activeTab === 'preset' ? 'text-touhou-red bg-white/60' : 'text-izakaya-wood/60 hover:bg-white/40'"
            >
                <LayoutGrid class="w-4 h-4" />
                选择预设模板
                <div v-if="activeTab === 'preset'" class="absolute bottom-0 left-0 right-0 h-0.5 bg-touhou-red"></div>
            </button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar p-6 bg-white/30 relative z-10">
            ...
            <div v-if="activeTab === 'custom'" class="space-y-5 animate-fade-in">
                <div class="grid grid-cols-2 gap-4">
                  <div class="space-y-2">
                    ...
                    <input v-model="formData.name" type="text" placeholder="您的名字" class="w-full px-4 py-2.5 bg-white/60 border border-izakaya-wood/10 rounded-xl text-sm outline-none focus:border-touhou-red transition-all">
                  </div>
                  <div class="space-y-2">
                    <label class="text-xs font-bold text-izakaya-wood/60">身份 (Identity)</label>
                    <input v-model="formData.identity" type="text" placeholder="例如: 异界魔法师" class="w-full px-4 py-2.5 bg-white/60 border border-izakaya-wood/10 rounded-xl text-sm outline-none focus:border-touhou-red transition-all">
                  </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button 
                            ...
                            :class="formData.power === rank ? 'bg-touhou-red text-white border-touhou-red shadow-md' : 'bg-white/40 text-izakaya-wood/60 border-izakaya-wood/10 hover:border-touhou-red/30'"
                        >
                            {{ rank }}
                        </button>
                        <select v-model="formData.power" class="px-3 py-1.5 bg-white/60 border border-izakaya-wood/10 rounded-lg text-xs outline-none focus:border-touhou-red ml-auto">
            ...
            <!-- Preset Tab -->
            <div v-if="activeTab === 'preset'" class="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
                <div 
                ...
                >
                    <div class="flex items-start gap-3">
                        <div class="p-3 bg-touhou-red/5 rounded-full text-touhou-red group-hover:bg-touhou-red group-hover:text-white transition-colors">
                            <component :is="preset.icon" class="w-6 h-6" />
                        </div>
                        <div class="flex-1">
                            <h4 class="font-bold text-izakaya-wood group-hover:text-touhou-red transition-colors">{{ preset.name }}</h4>
                            <p class="text-xs text-izakaya-wood/60 mt-1 line-clamp-2">{{ preset.desc }}</p>
                        </div>
                    </div>
                    <div class="mt-3 flex gap-2">
                        <span class="px-2 py-0.5 bg-izakaya-wood/5 rounded text-[10px] text-izakaya-wood/50 font-bold">
        ...
        <!-- Footer -->
        <div class="p-4 border-t border-izakaya-wood/10 bg-white/40 flex justify-end gap-3 relative z-10">
            <button 
                @click="$emit('close')"
