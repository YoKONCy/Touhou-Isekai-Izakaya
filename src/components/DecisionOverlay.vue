...
function submitVoteCreation() {
  ...
  const options = voteCreationOptions.value
    .split(',')
    .map((s) => s.trim())
    .filter((s) => s);
  if (options.length < 2) {
...
function handleToggleSharing() {
  ...
  addLog(
    newValue ? '你开启了 API 能源池共享' : '你关闭了 API 能源池共享',
    ...
    >
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <div
            v-if="gameStore.multiplayer.roomId"
            class="px-3 py-0.5 bg-touhou-red/40 border border-white/20 rounded-full text-xs flex items-center gap-1.5 ml-2"
          >
            <Users class="w-3 h-3" />
            <span class="opacity-80">房间:</span>
            <span class="font-bold">{{
              gameStore.multiplayer.roomName || gameStore.multiplayer.roomId
            }}</span>
          </div>
        </div>
      ...
      <nav class="flex bg-white/50 border-b border-izakaya-wood/10 relative z-10">
        <button
          ...
          :class="[
            'flex-1 py-3 flex items-center justify-center gap-2 font-display transition-all border-b-2',
            activeTab === 'action'
              ? 'border-touhou-red text-touhou-red bg-touhou-red/5'
      ...
      <main class="flex-1 overflow-hidden flex flex-col relative z-10">
        ...
        <div v-if="activeTab === 'action'" class="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div class="flex-1 flex flex-col p-4 border-r border-izakaya-wood/10 overflow-hidden">
                  ...
                  <div
                    ...
                    :class="[
                      ...
                      p.status === 'ready'
                        ...
                        : 'bg-white/60 border-izakaya-wood/10'
                    ]"
                  >
                    <div class="flex items-center justify-between">
                      <span
                        class="text-xs font-bold"
                        :class="p.isHost ? 'text-touhou-red' : 'text-izakaya-wood'"
                      >
                        {{ p.name }} {{ p.isHost ? '(房主)' : '' }}
                      </span>
                      <span
                        v-if="p.status === 'ready'"
                        class="text-[10px] bg-green-500 text-white px-1.5 py-0.5 rounded flex items-center gap-1"
                      >
                        <CheckCircle2 class="w-3 h-3" /> 就绪
                      </span>
                      <span
                        v-else-if="p.status === 'drafting'"
                        class="text-[10px] bg-yellow-500 text-white px-1.5 py-0.5 rounded animate-pulse"
                      >
                        正在输入...
                      </span>
                    </div>
            ...
            >
              ...
              <div
                v-if="!allReady"
                class="mt-2 text-[10px] text-touhou-red/60 flex items-center gap-1"
              >
          ...
          >
            <!-- 1. Dice Tools (命运之骰) -->
            <section
              class="flex-shrink-0 p-4 bg-white/60 rounded-2xl border border-izakaya-wood/10 shadow-sm"
            >
              <div class="flex items-center justify-between mb-3">
                ...
                <span
                  v-if="hasRolledThisTurn"
                  class="text-[10px] text-gray-400 font-bold bg-gray-100 px-2 py-0.5 rounded"
                >
                  本轮已投
                </span>
              </div>
              ...
              <div class="grid grid-cols-2 gap-2">
                ...
                >
                  <span class="text-[10px] font-bold text-gray-400 group-hover:text-touhou-red/60"
                    >D6</span
                  >
                  <div
                    class="w-8 h-8 flex items-center justify-center bg-gray-100 rounded-lg group-hover:bg-touhou-red/10"
                  >
              ...
              <div
                v-if="lastRollResult"
                class="mt-3 p-2 bg-touhou-red/5 rounded-lg border border-touhou-red/10 text-center animate-fade-in"
              >
                <span class="text-[10px] text-touhou-red/60 block uppercase font-bold"
                  >{{ lastRollResult.type }} 结果</span
                >
                <span class="text-2xl font-mono font-bold text-touhou-red">{{
            ...
            <section
              class="flex-shrink-0 p-4 bg-gradient-to-br from-yellow-500/10 to-orange-500/10 rounded-2xl border border-yellow-500/20 shadow-sm overflow-hidden relative"
            >
                  ...
                  <div v-if="gameStore.multiplayer.isHost" class="flex items-center gap-1.5 mr-1">
                    ...
                    <button
                      ...
                      :class="[
                        ...
                        gameStore.multiplayer.isSharingEnergy
                    ...
                    >
                      <div
                        :class="[
                          ...
                          gameStore.multiplayer.isSharingEnergy ? 'left-[18px]' : 'left-0.5'
                        ]"
                      ></div>
                    </button>
                  </div>
              ...
              <div v-if="!isContributing" class="space-y-2 animate-fade-in">
                <div class="flex justify-between items-end">
                  <span
                    :class="[
                      ...
                      totalEnergy < 0 ? 'text-red-600 animate-pulse' : 'text-yellow-600'
                    ]"
                  >
                    {{ totalEnergy }}
                  </span>
                  <span
                    :class="[
                      ...
                      totalEnergy < 0 ? 'text-red-700/60' : 'text-yellow-700/60'
                    ]"
                    >POINTS</span
                  >
                </div>
                <div class="h-2 bg-yellow-500/10 rounded-full overflow-hidden">
                  <div
                    ...
                    :style="{
                      width: Math.max(0, Math.min(100, (totalEnergy / 10000) * 100)) + '%'
                    }"
                  ></div>
                </div>
                <p
                  v-if="totalEnergy < 0"
                  class="text-[10px] text-red-500 font-bold italic animate-pulse"
                >
                  能源已透支！请尽快贡献能源。
                </p>
                <div v-else class="flex justify-between items-center">
                  ...
                  <span
                    v-if="gameStore.multiplayer.isSharingEnergy"
                    class="text-[9px] font-bold text-yellow-600 bg-yellow-500/10 px-1.5 py-0.5 rounded border border-yellow-500/20 animate-pulse"
                  >
                    已启用
                  </span>
                  <span
                    v-else
                    class="text-[9px] font-bold text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200"
                  >
                    未启用
                  </span>
                </div>
              ...
              <div v-else class="space-y-3 animate-fade-in-up">
                <div class="grid grid-cols-2 gap-2">
                  <button
                    ...
                    :class="[
                      'py-1.5 rounded-lg border text-xs font-mono transition-all',
            ...
            <!-- 3. Active Vote (进行中投票) -->
            <section
              v-if="gameStore.multiplayer.activeVote"
              class="flex-shrink-0 p-4 bg-white/80 rounded-2xl border-2 border-touhou-red/20 shadow-lg animate-bounce-in"
            >
              <h3 class="text-sm font-bold text-touhou-red mb-2 flex items-center gap-2">
                <Vote class="w-4 h-4" /> 进行中投票
              </h3>
              <p class="text-xs font-bold text-izakaya-wood mb-3 line-clamp-2">
                {{ gameStore.multiplayer.activeVote.proposal }}
              </p>
              ...
              <div class="space-y-2">
                <button
                  ...
                  :class="[
                    'w-full p-2 text-left text-xs rounded-lg border transition-all flex items-center justify-between',
                    gameStore.multiplayer.activeVote.votes[multiplayerService.identityKey] === idx
                      ? 'bg-touhou-red/10 border-touhou-red text-touhou-red'
                      : 'bg-white border-gray-100 hover:border-touhou-red/30'
                  ]"
                >
                  <span class="truncate">{{ opt }}</span>
                  <span class="font-mono text-[10px] opacity-60">
                    {{
                      Object.values(gameStore.multiplayer.activeVote.votes).filter((v) => v === idx)
                        .length
                    }}
                  </span>
                </button>
            ...
            <section class="flex-shrink-0 p-4 bg-white/40 rounded-xl border border-izakaya-wood/5">
              ...
              <div class="space-y-2">
                <div
                  v-for="p in players"
                  :key="p.id"
                  class="flex items-center justify-between text-xs group min-h-[24px]"
                >
                  <div class="flex items-center gap-2 flex-1 min-w-0">
                    <div
                      class="w-2 h-2 rounded-full flex-shrink-0"
                      :class="p.status === 'ready' ? 'bg-green-500' : 'bg-gray-300'"
                    ></div>
                    <span class="truncate">{{ p.name }}</span>
                  </div>
                  <div class="flex items-center gap-1 flex-shrink-0 ml-2">
                    <span
                      class="text-[10px] text-gray-400 px-1.5 py-0.5 bg-gray-100 rounded whitespace-nowrap"
                      >{{ p.isHost ? '房主' : '客机' }}</span
                    >
        ...
        >
          ...
          <div class="flex-1 flex flex-col p-4 border-r border-izakaya-wood/10 overflow-hidden">
            ...
            <div
              class="flex-1 bg-white/40 rounded-xl border border-izakaya-wood/10 overflow-y-auto p-3 space-y-3 mb-4 shadow-inner"
            >
              <div v-for="(msg, idx) in oocMessages" :key="idx" class="flex flex-col gap-0.5">
                <div class="flex items-center gap-2">
                  <span
                    ...
                    :class="
                      msg.senderId === multiplayerService.identityKey
                        ? 'text-touhou-red'
                        : 'text-blue-600'
                    "
                  >
                ...
                <p
                  class="text-sm bg-white p-2 rounded-lg border border-izakaya-wood/5 shadow-sm inline-block self-start max-w-[90%] break-words"
                >
                  {{ msg.content }}
                </p>
              </div>
              <div
                v-if="oocMessages.length === 0"
            ...
            <div class="flex gap-2">
              ...
              <button
                @click="sendChat"
                class="p-2 bg-izakaya-wood text-white rounded-lg hover:bg-izakaya-wood/80 transition-colors"
              >
                <Send class="w-5 h-5" />
              </button>
            </div>
            ...
            <div class="space-y-4">
              ...
              >
                ...
                <div class="flex items-center gap-3 mb-2">
                  <div
                    class="w-10 h-10 rounded-full border-2 border-touhou-red/20 overflow-hidden bg-gray-100"
                  >
                    <img
                      v-if="comp?.avatarUrl"
                      :src="comp.avatarUrl"
                  ...
                  <div>
                    <div class="text-sm font-bold text-izakaya-wood flex items-center gap-2">
                      ...
                      <span
                        v-if="comp?.isMe"
                        class="text-[10px] bg-touhou-red text-white px-1 py-0.5 rounded"
                        >我</span
                      >
                    </div>
                ...
                <div class="space-y-1.5">
                  ...
                  <div class="space-y-0.5">
                    <div class="flex justify-between text-[10px]">
                      <span class="flex items-center gap-1"
                        ><Activity class="w-3 h-3 text-red-500" /> HP</span
                      >
                      <span>{{ comp?.hp || 0 }}/{{ comp?.max_hp || 100 }}</span>
                    </div>
                    <div
                      class="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden border border-izakaya-wood/5"
                    >
                      <div
                        class="h-full bg-gradient-to-r from-red-500 to-pink-500 transition-all duration-500"
                        :style="{ width: `${((comp?.hp || 0) / (comp?.max_hp || 100)) * 100}%` }"
                      ></div>
                    </div>
                  </div>
                  <!-- 灵力值状态条 (MP Bar) 喵 -->
                  <div class="space-y-0.5">
                    <div class="flex justify-between text-[10px]">
                      <span class="flex items-center gap-1"
                        ><Zap class="w-3 h-3 text-blue-500" /> MP</span
                      >
                    ...
                    >
                      <div
                        ...
                        :style="{ width: `${((comp?.mp || 0) / (comp?.max_mp || 100)) * 100}%` }"
                      ></div>
                    </div>
                  </div>
                </div>
        ...
        <div v-if="activeTab === 'logs'" class="flex-1 flex flex-col p-6 overflow-hidden">
          <div class="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
            <div
              ...
              :key="idx"
              :class="[
                'p-3 rounded-xl border flex gap-3 items-start animate-fade-in',
                log.type === 'success'
                  ...
                  : log.type === 'warning'
                    ...
                    : log.type === 'error'
                      ...
                      : log.type === 'roll'
                        ? 'bg-touhou-red/5 border-touhou-red/20 text-touhou-red font-bold'
            ...
            >
              <div class="mt-0.5">
                ...
                <AlertCircle
                  v-else-if="log.type === 'warning' || log.type === 'error'"
                  class="w-4 h-4"
                />
    ...
    >
      <div class="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full border border-izakaya-wood/20">
        ...
        <p class="text-xs text-gray-500 mb-4">
          由
          {{ activeVote.initiatorId === multiplayerService.identityKey ? '我' : '其他玩家' }} 发起
        </p>
        ...
        <div
          v-if="activeVote.hasVoted"
          class="mt-4 text-center text-xs text-gray-400 animate-pulse"
        >
          等待其他玩家投票...
        </div>
      </div>
