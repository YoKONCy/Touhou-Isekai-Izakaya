  ...
  <div class="absolute bottom-10 right-20 pointer-events-auto perspective-1000" 
     ...
     <div v-else-if="currentMenu === 'spell'" class="relative w-80 min-h-[300px] flex flex-col gap-2 items-end animate-slide-in-right">
       ...
       <button 
          ...
          @mouseenter="$emit('hover-sound'); hoveredSpell = spell"
       ...
       >
          {{ spell.name }} <span class="text-xs ml-2" :class="canAffordSpell(spell) ? 'text-purple-300' : 'text-gray-600'">MP {{ getSpellCost(spell) }}</span>
       </button>
       
       <button @click="$emit('switch-menu', 'main')" @mouseenter="$emit('hover-sound')" class="mt-4 text-gray-400 hover:text-white font-bold italic transition-colors">BACK</button>
     </div>
     ...
     <div v-else-if="currentMenu === 'talk'" class="relative w-96 min-h-[300px] flex flex-col gap-2 items-end animate-slide-in-right">
       ...
       <div class="flex gap-4 w-full justify-end mt-2 items-center">
          <div v-if="playerPPoints < 15" class="text-red-500 text-xs font-bold bg-black/50 px-2 py-1 rounded border border-red-500/50">需要 15 P 点</div>
          <button 
       ...
       <div class="w-full h-[300px] overflow-y-auto pr-2 custom-scrollbar flex flex-col gap-2">
          ...
          >
             <div class="flex items-center gap-2">
                 ...
                 <span class="text-xs font-mono" :class="canAffordSkillP(skill) ? getSkillThemeClasses(skill.theme).textP : 'text-red-500'">{{ skill.costP }}P</span>
                 <span class="text-xs font-mono" :class="canAffordSkillAP(skill) ? 'text-blue-300' : 'text-red-500'">{{ skill.costAP }}AP</span>
             </div>
...
watch(() => props.talkInput, (v) => { talkInputModel.value = v; });
...
.clip-wedge { clip-path: polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%); }
.clip-rect-left { clip-path: polygon(10% 0, 100% 0, 100% 100%, 0% 100%); }
