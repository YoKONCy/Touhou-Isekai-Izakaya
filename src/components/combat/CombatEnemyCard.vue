  ...
  >
    ...
    >
      ...
      >
        ...
        <div
          class="absolute right-full top-0 mr-2 w-20 h-20 hidden group-hover:block z-50 border-2 border-purple-500 rounded bg-black"
        >
    ...
    <div
      ...
      :class="{
        ...
        'cursor-crosshair hover:scale-105 hover:brightness-125':
          selectionMode && en.hp > 0 && !isActing,
    ...
    >
      ...
      <div class="relative w-full h-full flex items-start justify-center animate-float-delayed">
        ...
        >
          <div
            class="absolute inset-0 rounded-b-3xl overflow-hidden backdrop-blur-sm border-t-4 border-purple-500 shadow-[0_0_30px_rgba(168,85,247,0.3)] bg-gradient-to-b from-purple-900/40 to-transparent z-0"
          ></div>
          <div
            class="absolute -top-4 left-0 w-full h-[115%] z-10 pointer-events-none flex items-start justify-center"
          >
            <img
          ...
          <div class="absolute inset-0 rounded-b-3xl overflow-hidden z-20 pointer-events-none">
            <div
              class="absolute inset-0 bg-gradient-to-b from-purple-600/60 to-transparent mix-blend-overlay"
            ></div>
            <span
              class="absolute top-10 left-0 w-full text-center text-2xl font-bold font-display text-white drop-shadow-[0_0_10px_rgba(168,85,247,0.8)] tracking-widest px-2 break-words"
            >
              {{ en.name }}
            </span>
          </div>
        ...
        <div v-else class="absolute inset-0 w-full h-full pointer-events-none">
          ...
          <div
            class="absolute inset-0 w-full h-full rounded-b-3xl backdrop-blur-sm border-t-4 border-purple-500 animate-shatter-2 overflow-hidden"
      ...
      <div
        ...
        :class="
          en.hp > 0 ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10 pointer-events-none'
        "
      >
        <div
        ...
        <div class="flex gap-2 justify-end w-full">
          <div
            class="bg-black/80 border-r-4 border-green-400 pr-2 pl-2 py-0.5 text-xs font-bold font-mono text-green-400 clip-hud-right flex items-center gap-1"
          >
            <span class="text-white"
              >{{ Math.round((getEnemyEffectiveDodge(en) || 0) * 100) }}%</span
            >
            <Zap class="w-3 h-3" />
          </div>
          <div
            class="bg-black/80 border-r-4 border-cyan-400 pr-2 pl-2 py-0.5 text-xs font-bold font-mono text-cyan-400 clip-hud-right flex items-center gap-1"
          >
        ...
        <div class="flex flex-wrap gap-1 justify-end max-w-[150px]">
          <div v-for="buff in en.buffs" :key="buff.id" class="relative group/buff">
            <div
              class="w-6 h-6 rounded bg-black/60 border border-gray-500 flex items-center justify-center text-[10px] text-white cursor-help overflow-hidden"
            >
              <span class="scale-75">{{ buff.name.substring(0, 1) }}</span>
            </div>
            <div
              class="absolute bottom-full right-0 mb-2 w-32 bg-black/90 border border-gray-600 text-white text-xs p-2 rounded pointer-events-none opacity-0 group-hover/buff:opacity-100 transition-opacity z-[100] shadow-xl text-right"
            >
              ...
              >
                <div
                  ...
                  :key="idx"
                  class="flex justify-between text-gray-400 text-[10px]"
                >
