<template>
  ...
  <transition name="intro-fade">
    <div v-if="showUltimate" class="fixed inset-0 z-[200] overflow-hidden pointer-events-none flex items-center justify-center font-display">
        ...
        <div class="absolute h-full w-full flex items-center z-20" :class="ultimateData.isPlayer ? 'justify-start' : 'justify-end'">
            <div class="relative h-full w-2/3 transition-all duration-500" :class="ultimateData.isPlayer ? 'animate-slide-in-left' : 'animate-slide-in-right'">
  ...
  >
    <div v-if="showSkill" class="fixed inset-0 z-[190] pointer-events-none flex flex-col justify-center overflow-hidden font-display">
        <!-- Strip Background -->
        <div class="absolute w-full h-40 md:h-56 bg-black/60 backdrop-blur-sm border-y-4 border-white/20 flex items-center animate-slide-in-fast origin-left"
             :class="[
                !skillData.isPlayer ? 'bg-gradient-to-l from-red-900/90 to-transparent' : 
                skillData.isSpecial ? 'bg-gradient-to-r from-amber-500 via-yellow-400 to-transparent' : 
             ...
             <div class="absolute top-[-50%] h-[200%] w-1/2 md:w-1/3 opacity-90 mix-blend-normal"
                  :class="skillData.isPlayer ? 'left-0 md:left-20 animate-slide-in-left' : 'right-0 md:right-20 animate-slide-in-right'">
                 <img :src="skillData.spriteUrl" class="h-full w-full object-cover object-top mask-image-fade-bottom" />
             </div>
     
             <!-- Text -->
             <div class="w-full px-10 md:px-32 flex flex-col justify-center relative z-10" :class="skillData.isPlayer ? 'items-end text-right' : 'items-start text-left'">
                 <div class="text-4xl md:text-6xl text-white font-black italic drop-shadow-[0_0_10px_rgba(255,255,255,0.5)] animate-fade-in-up">{{ skillData.spellName }}</div>
                 <div class="text-xl md:text-2xl text-white/80 font-serif mt-1 tracking-widest">{{ skillData.charName }}</div>
             </div>
        </div>
    </div>
  </transition>
  ...
  <Teleport to="body">
    <div v-if="showCombatFlow" class="fixed inset-0 z-[200] flex items-center justify-center overflow-hidden pointer-events-none font-display">
        ...
        <div class="absolute inset-0 flex items-center justify-center z-10 transition-all duration-1000"
          ...
          <img :src="playerSpriteUrl" class="h-[80vh] object-contain drop-shadow-[0_0_50px_rgba(168,85,247,0.8)] filter brightness-125 contrast-125 animate-float-slow" />
        </div>
...
.animate-portrait-move { animation: portraitMove 3s ease-out forwards; }
...
.animate-glitch-slam { animation: glitch-slam 0.8s cubic-bezier(0.22, 1, 0.36, 1) forwards; }
.animate-expand-width { animation: expand-width 0.8s ease-out forwards 0.5s; }
