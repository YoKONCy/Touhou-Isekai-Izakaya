        ...
        return obj
          .map((a) => {
            ...
            if (a.type === 'INVENTORY') {
              ...
              const itemName =
                typeof a.value === 'string' ? a.value : a.value?.name || a.value?.id || '未知物品';
...
<template>
  <div
    v-if="isOpen"
    class="fixed inset-0 z-50 flex items-center justify-center bg-izakaya-wood/60 backdrop-blur-sm animate-in fade-in duration-200"
  >
    <div
    ...
    >
      ...
      <div class="p-3 border-b border-izakaya-wood/10 flex gap-3 bg-white/40 relative z-10">
        <div class="relative flex-1 group">
          <Search
            class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-izakaya-wood/40 group-focus-within:text-touhou-red transition-colors"
          />
          <input
            v-model="searchQuery"
            type="text"
        ...
        <select
          v-model="filterType"
          class="px-3 py-2 text-sm border border-izakaya-wood/20 rounded-md bg-white/50 focus:outline-none focus:ring-2 focus:ring-touhou-red/20 focus:border-touhou-red/50 transition-all text-izakaya-wood cursor-pointer hover:bg-white/80"
        >
      ...
      <div class="flex-1 overflow-y-auto p-4 bg-white/20 space-y-3 relative z-10 custom-scrollbar">
        <div
          ...
          class="text-center py-10 text-izakaya-wood/40 flex flex-col items-center gap-2"
        >
        ...
        >
          <div class="flex items-start justify-between mb-2 pr-16">
            <div class="flex items-center gap-2">
              <span
                class="bg-izakaya-wood/5 text-xs px-2 py-1 rounded font-mono text-izakaya-wood/60 border border-izakaya-wood/5 flex items-center gap-1.5 group-hover:bg-touhou-red/5 group-hover:text-touhou-red transition-colors"
              >
                Turn {{ mem.turnCount }}
                <ExternalLink
              ...
              <span
                :class="[
                  ...
                  mem.type === 'summary'
                    ...
                    : mem.type === 'variable_change'
                      ? 'bg-purple-50 text-purple-700 border-purple-100'
                      : mem.type === 'alliance'
                        ? 'bg-amber-50 text-amber-700 border-amber-100'
                        : mem.type === 'intelligence'
          ...
          <div
            v-if="mem.gameDate || mem.gameTime || mem.location"
            class="flex flex-wrap gap-3 text-xs text-izakaya-wood/50 mb-3 font-serif pl-1 bg-izakaya-wood/5 p-1.5 rounded w-fit"
          >
          ...
          <div class="mt-3 flex items-end justify-between gap-4">
            <div class="flex flex-wrap gap-2">
              <span
                ...
                :key="tag"
                class="text-[10px] bg-izakaya-wood/5 text-izakaya-wood/60 px-1.5 py-0.5 rounded border border-izakaya-wood/10 hover:bg-izakaya-wood/10 transition-colors cursor-default"
              >
                #{{ tag }}
              </span>
              <span
                v-for="entity in mem.related_entities"
                :key="entity"
                class="text-[10px] bg-green-50 text-green-700 px-1.5 py-0.5 rounded border border-green-100 hover:bg-green-100 transition-colors cursor-default"
              >
                @{{ entity }}
              </span>
            </div>
            <span class="text-xs text-izakaya-wood/40 font-mono shrink-0">{{
    ...
    >
      <div
        class="bg-izakaya-paper w-full max-w-2xl max-h-[90vh] flex flex-col rounded-xl shadow-2xl border border-izakaya-wood/20 overflow-hidden animate-in zoom-in-95 duration-200"
      >
        <div
          class="p-4 border-b border-izakaya-wood/10 bg-white/50 flex justify-between items-center"
        >
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-white/30">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1">
              ...
              >
              <input
                v-model="editingMemory.gameDate"
                type="text"
                class="w-full px-3 py-2 bg-white/60 border border-izakaya-wood/10 rounded focus:border-blue-400 focus:ring-1 focus:ring-blue-400 outline-none text-sm font-serif text-izakaya-wood"
                placeholder="例如：纪元123年1月1日"
              />
            </div>
            <div class="space-y-1">
              <label class="text-xs font-bold text-izakaya-wood/60 uppercase tracking-wider"
              ...
              >
              <input
                v-model="editingMemory.gameTime"
                type="text"
                class="w-full px-3 py-2 bg-white/60 border border-izakaya-wood/10 rounded focus:border-blue-400 focus:ring-1 focus:ring-blue-400 outline-none text-sm font-serif text-izakaya-wood"
          ...
          <div class="space-y-1 pt-2">
            ...
            >
            <div
              class="flex flex-wrap gap-2 mb-2 p-2 bg-white/60 border border-izakaya-wood/10 rounded min-h-[42px] focus-within:border-blue-400 focus-within:ring-1 focus-within:ring-blue-400 transition-all"
            >
              <span
                v-for="(tag, index) in editingMemory.tags"
                :key="index"
                class="text-xs bg-izakaya-wood/10 text-izakaya-wood px-2 py-1 rounded flex items-center gap-1 border border-izakaya-wood/5 animate-in zoom-in duration-200"
              >
                #{{ tag }}
                <button
                  @click="removeTag(index)"
                  class="hover:text-red-500 hover:bg-red-100 rounded-full p-0.5 transition-colors ml-0.5"
                >
                  <X class="w-3 h-3" />
                </button>
              </span>
          ...
          <div class="space-y-3 pt-4 border-t border-izakaya-wood/10">
                ...
                <button
                  ...
                  :class="[
                    ...
                    editingMode === 'json'
                      ...
                      : 'text-izakaya-wood/50 hover:text-izakaya-wood/80'
                  ]"
            ...
            <div v-if="editingMode === 'json'" class="space-y-4">
              <div v-for="(val, key) in editingContentData" :key="key" class="space-y-1">
                ...
                <div
                  v-else
                  class="text-xs text-izakaya-wood/40 italic px-3 py-2 bg-black/5 rounded"
                >
                  复杂对象，请切换到源文本模式编辑
                </div>
              </div>
        ...
        <div class="p-4 border-t border-izakaya-wood/10 bg-white/50 flex justify-end gap-3">
          <button
            ...
            class="px-4 py-2 text-sm text-izakaya-wood/60 hover:bg-izakaya-wood/5 rounded transition-colors"
          >
            取消
          </button>
          <button
            @click="saveEdit"
            class="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-200 rounded transition-all flex items-center gap-2"
          >
            <Save class="w-4 h-4" />
            保存修改
          </button>
