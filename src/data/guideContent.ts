...
> **提示**：遇到强敌打不过？先去找小妖怪切磋一下，提升几级熟练度再回来试试吧！
`
  },
  {
    id: 'memory',
...
- **视觉沉浸**：精美的日系生活感插画，让您的幻想乡之旅更具代入感。

### 如何配置？
绘图系统的配置分为两个步骤，均在 **[⚙️ 设置](action:openSettings)** -> **绘图配置** 选项卡中完成：
...
- **API 格式**：目前 **仅支持 OpenAI 格式** 的绘图接口接入（即符合 \`/v1/chat/completions\` 或其多模态接口标准的 API）。
- **Base URL**：填入您的绘图 API 基础地址（例如 \`https://api.your-provider.com/v1\`）。
- **API Key**：填入对应的授权密钥。
- **模型名称**：填入绘图模型的具体 ID（例如 \`Gemini2.5-flash-image\`）。

### ⚠️ 注意事项
1. **Token 消耗**：开启此功能会增加 API 调用频率并消耗额外的 Token，请根据您的预算决定是否开启。
2. **预览版说明**：目前绘图系统仍处于测试阶段，生成图片的质量和速度很大程度上取决于您所使用的 API 供应商。
3. **网络环境**：由于图片数据较大，请确保您的网络环境稳定，以避免图片加载失败。
...
### 核心玩法
1. **动态地图生成**：
    - 每次营业，LLM 都会根据当前的剧情背景（如：夏祭、大雪）为您生成独特的居酒屋布局。
    - 您可以控制角色在店内移动，与家具和客人交互。

2. **自由烹饪系统**：
    - 在厨房区域与灶台交互，您可以从背包中任意挑选食材投入锅中。
    - **没有固定配方**！LLM 会根据您投入的食材实时判定菜名、口味和品质。

3. **顾客互动与评价**：
    - 幻想乡的 NPC 们会陆续光顾。
    - 满足她们的点单需求，并接受她们（由 LLM 驱动）的毒舌或夸奖。
    - 良好的经营表现将为您带来大量的 **金钱**、**声望** 以及 **人物好感度**。

### ⚠️ 重要说明
由于该系统目前处于 **高度开发阶段 (Alpha)**：
- **功能限制**：目前的顾客寻路逻辑和结算系统尚不完善。
